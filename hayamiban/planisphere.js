...
(function (root) {
  ...
  const Dt = isNode ? require('../data.js') : { CONS, LONE, MW, LOCS };
  ...
  function model({ lat, lon, magLimit = 4.5 }) {
    ...
    const rh = Math.max(G.R_HORIZON, G.R_MAP * (180 - lat) / 180 + 0.3);
    const span = (180 - lat) * G.R_MAP / rh;                // 北極から R_MAP までの角度（度）
    return { lat, lon, magLimit, rh, span, decEdge: 90 - span, k: G.R_MAP / span };
  }
  /** 赤緯（度）→ 中心からの距離（mm）。等距離方位図法 */
  const rOf = (m, dec) => (90 - dec) * m.k;
  /** 中心からの距離（mm）→ 赤緯（度） */
  const decOf = (m, r) => 90 - r / m.k;
  ...
  /** 日付の目盛りの角度（星図盤の上。度）: その日の夜（翌日の日本時間 0 時）に、時刻の目盛り 0 時と合う位置。
   *  a = τ(0時) − 地方恒星時 = −(グリニッジ恒星時×15) − 135°（経度によらない）。4 年の平均 */
  function dateAngle(month, day) {
    let sx = 0, sy = 0;
    for (const Y of REF_YEARS) {
      const ms = Date.UTC(Y, month - 1, day + 1, -JST_OFFSET_H);
      const a = (-A.gmstHours(ms) * 15 - JST_MERIDIAN) * DG;
      sx += Math.cos(a); sy += Math.sin(a);
    }
    return norm360(Math.atan2(sy, sx) / DG);
  }
  /** 夜の 0 時からの時間（時。昼の 12 時で切る: 12〜24 時は −12〜0、0〜12 時は 0〜12） */
  const fromMidnight = hours => (((hours + 12) % 24) + 24) % 24 - 12;
  /** 時刻の目盛りの角度（上の盤の上。度）: 日本時間 hours 時。経度の差の分だけ回す */
  const timeAngle = (m, hours) => fromMidnight(hours) * SIDEREAL_DEG_PER_H + (m.lon - JST_MERIDIAN);

  /** 日本時間の年月日・時（小数） */
  function jstParts(ms) {
    const d = new Date(ms + JST_OFFSET_H * 3600000);
    return { y: d.getUTCFullYear(), mo: d.getUTCMonth() + 1, d: d.getUTCDate(),
      h: d.getUTCHours() + d.getUTCMinutes() / 60 + d.getUTCSeconds() / 3600 };
  }
  /** その時刻が属する夜の日付（日本時間の 12 時より前は前の日の夜） */
  function nightOf(ms) {
    const p = jstParts(ms), n = p.h < 12 ? jstParts(ms - 12 * 3600000) : p;
    return { y: n.y, mo: n.mo, d: n.d, h: p.h };
  }
  /** 盤の回転 ψ（度）: その夜の日付の目盛りを、その時刻の目盛りに合わせたとき。
   *  2 月 29 日の夜は目盛りが無いので 2 月 28 日に合わせる */
  function rotationFor(m, ms) {
    const n = nightOf(ms);
    const day = n.mo === 2 && n.d === 29 ? 28 : n.d;
    return norm360(timeAngle(m, n.h) - dateAngle(n.mo, day));
  }

  /** 赤道座標（J2000、度）→ 星図盤の上の点（mm、中心が原点） */
  const diskXY = (m, raDeg, dec) => polar(rOf(m, dec), -raDeg);
  /** 組み立てた盤（上の盤の座標）の上の点 */
  const skyXY = (m, raDeg, dec, psi) => polar(rOf(m, dec), -raDeg + psi);
  /** 上の盤の上の点 → [時角 H（度、西が正）, 赤緯（度）] */
  function maskToHaDec(m, x, y) {
    return [Math.atan2(x, y) / DG, decOf(m, Math.hypot(x, y))];
  }
  ...
  function starLabel(s) {
    if (!s.name) return '';
    if (/北極星/.test(s.name)) return '北極星';
    if (s.flag === 'cl') return s.name.split('（')[0];
    return s.mag <= 1.6 ? s.name.split('（')[0] : '';
  }
  ...
  function diskSvg(m, { rot = 0 } = {}) {
    const o = [], clip = 'hb-clip' + (++clipSeq);
    const P = (ra, dec) => polar(rOf(m, dec), -ra + rot);
    ...
    for (let dec = 60; dec > m.decEdge; dec -= 30) {
      o.push(`<circle r="${f1(rOf(m, dec))}" fill="none" stroke="${COL.grid}" stroke-width="${dec === 0 ? 0.3 : 0.15}"${dec === 0 ? '' : ' stroke-dasharray="1 1"'}/>`);
    }
    for (let h = 0; h < 24; h += 2) {
      const [x0, y0] = P(h * 15, 80), [x1, y1] = P(h * 15, m.decEdge);
    ...
    { const [x, y] = P(40, 18); o.push(radialText(x, y, -40 + rot, 2.1, '黄道', ` fill="${COL.ecl}"`)); }
    { const [x, y] = P(210, -2.2); o.push(radialText(x, y, -210 + rot, 2.1, '天の赤道', ` fill="${COL.dim}"`)); }
    // 星座の線
    const stars = allStars();
    const lines = [];
    Dt.CONS.forEach(c => c.ln.forEach(([i, j]) => lines.push(pathOf(greatArc(c.pts[i], c.pts[j], 6).map(p => P(p.ra, p.dec))))));
    o.push(`<path d="${lines.join('')}" fill="none" stroke="${COL.line}" stroke-width="0.3" stroke-linejoin="round"/>`);
    // 星
    const dots = [];
    for (const s of stars) {
      if (s.dec < m.decEdge - 1) continue;
    ...
    // 名前（星座と明るい星）。中心向きが上
    Dt.CONS.forEach(c => {
      const cc = conCenter(c); if (cc.dec < m.decEdge + 2) return;
      const [x, y] = P(cc.ra, cc.dec); const a = -cc.ra + rot;
      o.push(radialText(x, y, a, 2.7, c.jp, ` fill="${COL.name}" font-weight="700" stroke="#fff" stroke-width="0.5" paint-order="stroke"`));
    });
    for (const s of stars) {
      const t = starLabel(s); if (!t || s.dec < m.decEdge + 2) continue;
      const [x, y] = P(s.ra, s.dec), a = -s.ra + rot;
    ...
    for (let mo = 1; mo <= 12; mo++) {
      ...
      const a = dateAngle(mo, 16) + rot, [x, y] = polar(r0 + 10, a);
      o.push(radialText(x, y, a, 3.8, mo + '月', ` fill="${COL.ink}" font-weight="700"`));
    }
  ...
  function maskSvg(m, { mode = 'print', place = '' } = {}) {
    const o = [];
    const hz = horizon(m);
    const rN = rOf(m, 90 - m.lat);                         // 北の地平線（中心の真上）
    if (mode === 'view') {
      ...
      o.push(`<text y="${f1(rOf(m, m.lat) + 6)}" font-size="3.4" text-anchor="middle" fill="${COL.dim}">切りぬく</text>`);
    }
    ...
    for (let h = 0; h < 24; h++) {
      ...
      o.push(radialText(x, y, a, night ? 2.9 : 2.4, h + '時', ` fill="${night ? ink : dim}"${night ? ' font-weight="700"' : ''}`));
    }
    // 題と凡例（北の地平線より上）
    if (mode === 'print') {
      const top = -rN - 7;
      o.push(`<text y="${f1(top - 20)}" font-size="6.2" text-anchor="middle" font-weight="700" fill="${ink}" letter-spacing="0.8">星座早見盤</text>`);
      if (place) o.push(`<text y="${f1(top - 13.5)}" font-size="3.1" text-anchor="middle" fill="${ink}">${esc(place)}</text>`);
      o.push(`<text y="${f1(top - 9)}" font-size="2.5" text-anchor="middle" fill="${dim}">北緯 ${f1(m.lat)}°・東経 ${f1(m.lon)}°／時刻は日本時間（経度の差を補正ずみ）</text>`);
      const leg = [-1, 0, 1, 2, 3, 4].filter(v => v <= m.magLimit);
      const w = 11.5, x0 = -((leg.length - 1) * w) / 2 - 2;
      leg.forEach((v, i) => {
        const x = x0 + i * w, r = starRadius(v);
        o.push(`<circle cx="${f1(x)}" cy="${f1(top - 3.2)}" r="${f1(r)}" fill="${ink}"/><text x="${f1(x + r + 0.7)}" y="${f1(top - 3.2)}" font-size="2.3" dominant-baseline="central" fill="${dim}">${v < 0 ? '−' + -v : v}等</text>`);
      });
    }
    return o.join('\n');
  }
...
<text x="-6" y="-2">割りピン</text><path d="M4 -2.8 L23.8 -3.3" stroke="${COL.dim}" stroke-width="0.2"/>
  ...
  function sheets({ lat, lon, magLimit = 4.5, layout = 'two', place = '', credit = true }) {
